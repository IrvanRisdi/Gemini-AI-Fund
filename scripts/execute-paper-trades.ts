...
async function main() {
  const ledger = read<Ledger>(LEDGER); const scan = read<{ candidates?: Candidate[] }>(SCAN); const state = read<{ agents?: Record<string, { status: string; last_action?: string }> }>(STATE); const prices = await fetchBulkIdrPrices(); const timestamp = now();
  for (const [agent, book] of Object.entries(ledger.agents)) {
    ...
    for (const order of book.pendingOrders.filter((item) => item.status === 'pending')) {
      ...
      if (price > 0 && price <= order.stopPrice) { order.status = 'cancelled'; continue; }
      const shouldFill = order.type === 'limit' ? price >= order.entryLow && price <= order.entryHigh : price >= order.entryHigh;
      if (shouldFill) fill(book, order, price, timestamp);
    }
