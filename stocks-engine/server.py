...
STRATEGIES = {
    "swing": {"objective":"Menangkap tren menengah pada saham likuid tanpa mengejar harga yang terlalu jauh.","timeframes":"Daily untuk setup · Weekly untuk regime","entry_rules":["Close di atas EMA20 dan EMA50","RSI 52–66 dan RVOL minimal 1,25×","Jarak harga maksimum 8% dari EMA20"],"exit_rules":["Stop di bawah swing low atau ATR struktural","Target sekitar 2R sebelum tick/fee","Tidak averaging down"],"no_trade":["Data stale","Harga terlalu jauh dari EMA20","IHSG risk-off ekstrem"]},
    "scalping": {"objective":"Menangkap momentum intraday 5 menit yang masih memiliki edge bersih setelah biaya.","timeframes":"5m untuk setup dan eksekusi delayed-paper","entry_rules":["Harga > EMA20 > EMA50","RVOL ≥2× dan RSI 55–66","Momentum 15m 0,35–1,25% dan jarak VWAP 0,15–0,80%"],"exit_rules":["Stop ATR/tick; risiko equity 1%","Target minimal 1,5R bersih setelah fee","Time stop 90 menit aktif"],"no_trade":["Candle stale atau di luar jam kontinu IDX","Expected edge setelah biaya tidak memadai","Volume lemah atau harga terlalu jauh dari VWAP"]},
    "open-low": {"objective":"Memilih saham dengan kekuatan pembukaan ketika open bertahan sebagai low hari berjalan.","timeframes":"5m · hanya 45 menit perdagangan aktif pertama","entry_rules":["Low sesi tidak lebih dari satu tick di bawah open sesi","RVOL ≥1,75×, RSI 55–70, dan close di atas VWAP","Kenaikan dari open 0,30–2,50%"],"exit_rules":["Stop satu tick di bawah open; risiko equity 1,5%","Target minimal 1,5R bersih setelah fee","Keluar akhir sesi"],"no_trade":["Di luar 45 menit aktif pertama","Open=low karena belum aktif diperdagangkan","Candle stale atau lonjakan berlebihan"]},
    ...
    "breakout-retest": {"objective":"Membeli breakout valid setelah resistance diuji kembali dan bertahan sebagai support.","timeframes":"Daily setup dan retest","entry_rules":["Close sebelumnya melewati resistance lalu retest valid","Trend EMA20/50, RVOL ≥1,5×, RSI 50–68","Kenaikan harian tidak melebihi 5%"],"exit_rules":["Stop di bawah retest low; risiko equity 2%","Target minimum 1,5R bersih","Plan kedaluwarsa bila level lama terlewati"],"no_trade":["Deep retest/hard reject","Harga atau RSI terlalu panas","Breakout tanpa volume"]},
}
