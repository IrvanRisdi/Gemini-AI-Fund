...
import type { LatestScan } from '@/lib/desk-data';

type SortKey = 'rank' | 'symbol' | 'marketCap' | 'price' | 'change' | 'volume';
type SortDir = 'asc' | 'desc';
type Filter = 'all' | 'watchlist' | 'scan' | 'uptrend' | 'gainers' | 'losers' | 'volume';
const PAGE_SIZE = 50;
...
function scanSymbol(pair: string): string {
  return pair.replace('/', '').replace('_', '').replace(/idr$/i, '').toLowerCase();
}

export function PairExplorer({ pairs, updatedAt, latestScan }: { pairs: ExplorePairRow[]; updatedAt: string; latestScan: LatestScan | null }) {
  ...
  const diagnosticBySymbol = useMemo(() => new Map((latestScan?.diagnostics ?? []).map((item) => [scanSymbol(item.pair), item])), [latestScan]);
  const scannedSymbols = useMemo(() => new Set((latestScan?.universe ?? []).map((item) => scanSymbol(item.pair))), [latestScan]);
  ...
  const market = useMemo(() => {
    ...
    const uptrends = [...diagnosticBySymbol.values()].filter((item) => item.status === 'uptrend').length;
    return { gainers, losers, topGainer, topVolume, uptrends };
  }, [pairs, diagnosticBySymbol]);
  ...
  const filtered = useMemo(() => {
    ...
    return pairs.filter((pair) => {
      ...
      const matchesFilter = filter === 'all' || (filter === 'watchlist' && watchlist.includes(pair.symbol)) || (filter === 'scan' && scannedSymbols.has(pair.symbol)) || (filter === 'uptrend' && diagnosticBySymbol.get(pair.symbol)?.status === 'uptrend') || (filter === 'gainers' && (pair.globalChangePct24h ?? 0) > 0) || (filter === 'losers' && (pair.globalChangePct24h ?? 0) < 0) || (filter === 'volume' && pair.volumeIdr != null);
      return matchesSearch && matchesFilter;
    });
  }, [pairs, query, filter, watchlist, scannedSymbols, diagnosticBySymbol]);
